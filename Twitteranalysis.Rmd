...
```{r}
set.seed(321)
gloss_token <- as.data.frame(gloss_df$text)
gloss_token <- gloss_token %>% mutate(id = row_number())
gloss_token <- gloss_token[, c(2,1)]
gloss_token$`gloss_df$text` <- as.character(gloss_token$`gloss_df$text`)
gloss_btm <- BTM(gloss_token, k = 5, beta = 0.01, iter = 1000, trace = 100)
terms(gloss_btm, top_n = 10)
```